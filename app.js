...
const screens = {
  ...
  match: () => { const position=state.creator%state.creatorOrder.length, c=creators[state.creatorOrder[position]], fit=fitSignals(c); return `<div class="discover-top"><button class="campaign-switch"><span>Peakline · Summer Training</span>⌄</button><span class="counter">${position+1} of ${creators.length} creators</span></div>
    ...
    <article class="profile-stream" id="creatorCard">
      ...
      <div class="profile-decision-zone"><div class="swipe-guide" id="swipeGuide"><span>← Pass</span><strong>Swipe or use the buttons</strong><span>Select →</span></div><span class="browser-column-cue pass-cue">← Pass</span><span class="browser-column-cue select-cue">Select →</span>
    ...
    <aside class="decision-rail"><div class="decision-score"><span>Campaign fit</span><strong>${c.score}<small>/100</small></strong><b>Top match</b></div><div class="desktop-creator-stats" aria-label="Creator stats"><div><strong>${c.subs}</strong><small>Subscribers</small></div><div><strong>${c.views}</strong><small>Avg. views</small></div><div><strong>${c.eng}</strong><small>Engagement</small></div></div><div class="format-recommendation"><span>YouTube activation</span><strong>${c.format}</strong><small>${c.formatNote}</small></div><section class="desktop-ai-panel"><div class="brand-badges">${geminiBadge('Gemini Multimodal API')}</div><h2>Video evidence</h2><div class="mini-proof"><img src="${c.videoImage}" alt="${c.videoTitle}"><span><b>${c.timestamp}</b><strong>${c.videoTitle}</strong><small>${c.moment} · Open proof ↗</small></span></div><article><span>Vision · ${c.visualFit}% visual fit</span><p>${c.visual}</p></article><article><span>Transcript & audio · ${c.transcriptFit}% message fit</span><p>${c.transcript}</p></article></section><button class="brief-generate" data-open-brief>⚡ Generate AI Brief</button></aside>
    </div>`},
